...
const many = pokémon.filter(p => p.types.length > 1);
console.log(many);
...
const poison = pokémon.filter(p => p.types.length == 1 && p.types.includes("poison")).map(p => p.name);
console.log(poison);


const flying2nd = pokémon.filter(p => p.types.length == 2 && p.types[1]=="flying").map(p => p.types[0]);
console.log(flying2nd);
